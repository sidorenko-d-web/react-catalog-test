import type { IColor, IItem, ISizeRespose } from '../types'
...
function getSizes(): Promise<ISizeRespose[]> {
  return new Promise((resolve) => {
    setTimeout(() => resolve(sizes), 250)
  })
}

function getSize(id: number): Promise<ISizeRespose> {
...
function getProducts(): Promise<IItem[]> {
  return new Promise((resolve) => {
    setTimeout(() => resolve(products as IItem[]), 250)
  })
}

function getProduct(id: number): Promise<IItem> {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
...
function getProductColor(productID: number, colorID: number): Promise<IColor> {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
